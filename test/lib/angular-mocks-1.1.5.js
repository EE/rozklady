...
angular.mock.$ExceptionHandlerProvider = function() {
   ...
   *                means that there is a bug in the controllers or test, so this mock will
 ...
 * controllers make requests, respond with pre-trained responses and assert that the end result is
 * what we expect it to be.
 *
 * This mock implementation can be used to respond with static or dynamic responses via the
 * `expect` and `when` apis and their shortcuts (`expectGET`, `whenPOST`, etc).
 *
 * When an Angular controllers needs some data from a server, it calls the $http service, which
 ...
 *
 * Request expectations provide a way to make assertions about requests made by the controllers and
 * to define responses for those requests. The test will fail if the expected requests are not made
 * or they are made in the wrong order.
 *
 * Backend definitions allow you to define a fake backend for your controllers which doesn't assert
 ...
 *
 * As opposed to unit-testing, in an end-to-end testing scenario or in scenario when an controllers
 ...
 *
 * To setup the controllers to run with this http backend, you have to create a module that depends
 * on the `ngMockE2E` and your controllers modules and defines the fake backend:
 *
 * <pre>
