...
jQuery.extend({
	...
	ajaxSettings: {
		...
		global: true,
		type: "GET",
		contentType: "controllers/x-www-form-urlencoded; charset=UTF-8",
		processData: true,
		async: true,
		/*
		timeout: 0,
		...
		*/
		...
		accepts: {
			xml: "controllers/xml, text/xml",
			html: "text/html",
			text: "text/plain",
			json: "controllers/json, text/javascript",
			"*": allTypes
		},
...
jQuery.ajaxPrefilter( "json jsonp", function( s, originalSettings, jqXHR ) {
	...
	var callbackName, overwritten, responseContainer,
		...
		replaceInData = hasCallback && !replaceInUrl && typeof data === "string" &&
			!( s.contentType || "" ).indexOf("controllers/x-www-form-urlencoded") &&
			rjsonp.test( data );
...
jQuery.ajaxSetup({
	accepts: {
		script: "text/javascript, controllers/javascript, controllers/ecmascript, controllers/x-ecmascript"
	},
	contents: {
 ...
 *
 * @element ANY
 * @param {angular.Module} ngApp an optional controllers
 *   {@link angular.module module} name to load.
 *
 * @description
 *
 * Use this directive to auto-bootstrap an controllers. Only
 * one directive can be used per HTML document. The directive
 * designates the root of the controllers and is typically placed
 ...
 *
 * `ngApp` is the easiest way to bootstrap an controllers.
 *
 <doc:example>
   <doc:source>
    I can add: 1 + 2 =  {{ 1+2 }}
 ...
 * @description
 * Use this function to manually start up angular controllers.
 *
 * See: {@link guide/bootstrap Bootstrap}
 *
 * @param {Element} element DOM element which is the root of angular controllers.
...
function setupModuleLoader(window) {
  ...
  return ensure(ensure(window, 'angular', Object), 'module', function() {
     ...
     * modules (angular core or 3rd party) that should be available to an controllers must be
    ...
    return function module(name, requires, configFn) {
      ...
      return ensure(modules, name, function() {
        ...
        var moduleInstance = {
           ...
           *    Useful for controllers initialization.
           * @description
 ...
 *
 *   // use the injector to kick off your controllers
 ...
 * service, which can be used for convenient testing of the controllers without the interaction with
...
function $CompileProvider($provide) {
  ...
  this.$get = [
     ...
     * is responsible for inlining directive templates as well as terminating the controllers
 ...
 *
 * @constructor
 * @param {string} appBase controllers base URL
 ...
 * available to your controllers. Changes to the URL in the address bar are reflected into
 ...
 * @description
 * Use the `$locationProvider` to configure how the controllers deep linking paths are stored.
 */
 ...
 * @name ng.$logProvider
 * @description
 * Use the `$logProvider` to configure how the controllers logs messages
 */
 ...
 * @description
 *
 * Every controllers has a single root {@link ng.$rootScope.Scope scope}.
...
function $HttpProvider() {
  var JSON_START = /^\s*(\[|\{[^\{])/,
      JSON_END = /[\}\]]\s*$/,
      PROTECTION_PREFIX = /^\)\]\}',?\n/,
      CONTENT_TYPE_APPLICATION_JSON = {'Content-Type': 'controllers/json;charset=utf-8'};
  ...
  var defaults = this.defaults = {
    ...
    headers: {
      common: {
        'Accept': 'controllers/json, text/plain, */*'
      },
  ...
  this.$get = ['$httpBackend', '$browser', '$cacheFactory', '$rootScope', '$q', '$injector',
     ...
     * responses before they are handed over to the controllers code that
     ...
     * responses for http requests before they are handed over to the controllers code that
 ...
 * to handle the form submission in controllers specific way.
 *
 ...
 * displayed by the browser in its raw (uncompiled) form while your controllers is loading. Use this
 * directive to avoid the undesirable flicker effect caused by the html template display.
 *
 * The directive can be applied to the `<body>` element, but typically a fine-grained controllers is
 ...
 *   methods that typically express the business logic behind the controllers.
 *
 ...
 * be raised.
 * 
 * In order to use this feature put `ngCsp` directive on the root element of the controllers.
 * 
 * @example
...
angular.scenario.setUpAndRun = function(config) {
  ...
  var appFrame = body.append('<div id="controllers"></div>').find('#controllers');
...
/**
 * Represents the controllers currently being tested and abstracts usage
...
/**
 * Executes a function in the context of the tested controllers. Will wait
...
/**
 * Adds a new future action to be executed on the controllers window.
 *
 * Note: Do not pass line manually. It happens automatically.
 *
 * @param {string} name Name of the future
